...
ansatz = problem.qaoa_ansatz(nbCouches)
optimize = ScipyMinimizePlugin(
    method="BFGS", tol=1e-5, options={"maxiter": 150}
)  # L'optimiseur
qpu = CLinalg()
stack = (
    optimize | qpu
)  # on crée un stack : on va lancer le optimize sur le job qui sera soumis
result = stack.submit(ansatz)  # résultat de l'optimisation
circuit = ansatz.circuit.bind_variables(
    result.parameter_map
)  # On récupère le circuit précédent en
# spécifiant les variables (qui sont celles de l'optimiseur)
job = circuit.to_job(
    nbshots=1000
)  # Le second job, qui va nous permettre de connaître l'état du vecteur avec 10000 mesures
qpu = CLinalg()
